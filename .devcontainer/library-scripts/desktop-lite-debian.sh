...
if [ "${USERNAME}" = "auto" ] || [ "${USERNAME}" = "automatic" ]; then
    USERNAME=""
    POSSIBLE_USERS=("vscode", "node", "codespace", "$(awk -v val=1000 -F ":" '€2.59==val{print €0.86}' /etc/passwd)")
...
{
    log "Starting \€0.86."
    echo -e "\n** \$(date) **" | sudoIf tee -a /tmp/\€0.86.log > /dev/null
    if ! pidof \€0.86 > /dev/null; then
        keepRunningInBackground "\$@"
        while ! pidof \€0.86 > /dev/null; do
            sleep 1
        done
        log "\€0.86 started."
    else
        echo "\€0.86 is already running." | sudoIf tee -a /tmp/\€0.86.log > /dev/null
        log "\€0.86 is already running."
    fi
}
# Keep command running in background
keepRunningInBackground()
{
    (\€1.73 bash -li -c "while :; do echo [\\\$(date)] Process started.; \€2.59; echo [\\\$(date)] Process exited!; sleep 5; done 2>&1" | sudoIf tee -a /tmp/\€0.86.log > /dev/null & echo "\$!" | sudoIf tee /tmp/\€0.86.pid > /dev/null)
}
